...
import { Upload, X, MapPin, DollarSign, Home, List, FileText, Check, AlertCircle, PlayCircle, Loader2 } from 'lucide-react';
...
const CreateRoom = () => {
    ...
    const extractCoordinatesFromUrl = (url) => {
        try {
            ...
            // Pattern 4: /place/ with coordinates
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await api.post('/rooms/new', data);

            toast.success('Room listed successfully! Pending admin approval.');
            navigate('/dashboard');
        } catch (error) {
                    ...
                    <div className="p-8 md:p-12">
                        <header className="mb-10">
                            <h1 className="text-3xl font-extrabold text-slate-900">
                                {step === 1 ? 'Basic Information' : step === 2 ? 'Location & Features' : 'Final Details & Media'}
                            </h1>
                            <p className="text-slate-500 mt-2">Fill in the details to list your room on SmartRoom</p>
                        </header>
                        ...
                        >
                                ...
                                <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            ...
                                            onChange={handleChange}
                                            rows="4"
                                            placeholder="Describe your room, light, air, and vibes..."
                                ...
                                <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                                            ...
                                            <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
                                                <div className="p-6 border-b border-slate-100 flex items-center justify-between">
                                                    <div>
                                                        ...
                                                        <p className="text-sm text-slate-500 mt-1">Click anywhere on the map to set your room's location</p>
                                                    </div>
                                                    <button
                                    ...
                                    {formData.location.city && formData.location.address && (
                                        <div className="animate-in fade-in slide-in-from-top-2 duration-500">
                                            <label className="text-xs font-bold text-slate-400 uppercase tracking-widest block mb-3">Live Location Preview</label>
                                            <div className="rounded-3xl overflow-hidden h-48 border border-slate-200 bg-slate-50 relative group">
                                                <iframe
                                                    width="100%"
                                                    height="100%"
                                                    frameBorder="0"
                                                ...
                                                ></iframe>
                                                <div className="absolute bottom-2 right-2 bg-white/90 backdrop-blur px-3 py-1.5 rounded-xl border border-slate-200 flex items-center space-x-2 shadow-sm pointer-events-none">
                                                    <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
                                                    <span className="text-[10px] font-bold text-slate-600">Maps Link Active</span>
                                                </div>
                                            </div>
                                            <p className="mt-2 text-[10px] text-slate-400 italic">
                                                * This is how guests will see your room's location. Please provide a clear address.
                                            </p>
                                        </div>
                                    )}
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {formData.facilities.map((fac, idx) => (
                                                <span key={idx} className="flex items-center space-x-2 bg-slate-100 px-3 py-1.5 rounded-full text-sm font-medium text-slate-700">
                                                    <span>{fac}</span>
                                                    <button onClick={() => removeFacility(idx)} className="text-slate-400 hover:text-red-500">
                                                        <X size={14} />
                                                    </button>
                                                </span>
                                            ))}
                            ...
                            {step === 3 && (
                                <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div>
                                            <label className="text-sm font-bold text-slate-700 block mb-2">Upload Images (Max 5)</label>
                                            <label className="border-2 border-dashed border-slate-200 rounded-3xl p-6 flex flex-col items-center justify-center text-slate-400 hover:border-primary-400 hover:bg-primary-50 transition-all cursor-pointer group">
                                                ...
                                                <p className="text-xs font-medium text-center">Select Images</p>
                                                <input
                                                    type="file"
                                                    name="images"
                                                    multiple
                                            ...
                                            <div className="flex gap-2 mt-2 overflow-x-auto pb-2">
                                                {imagePreviews.map((url, i) => (
                                                    <img key={i} src={url} alt="preview" className="w-16 h-16 object-cover rounded-lg border border-slate-200" />
                                                ))}
                                            </div>
                                        </div>
                                        <div>
                                            <label className="text-sm font-bold text-slate-700 block mb-2">Upload Video (Max 1)</label>
                                            <label className="border-2 border-dashed border-slate-200 rounded-3xl p-6 flex flex-col items-center justify-center text-slate-400 hover:border-primary-400 hover:bg-primary-50 transition-all cursor-pointer group">
                                                <Video size={40} className="mb-2 group-hover:scale-110 transition-transform" />
                                                <p className="text-xs font-medium text-center">Select Video</p>
                                                <input
                                                    type="file"
                                                    name="videos"
                                                    accept="video/*"
                                            ...
                                            {videos.length > 0 && <p className="text-xs mt-2 text-green-600 font-medium">Video selected: {videos[0].name}</p>}
                                        </div>
                                    </div>
                                </div>
                            )}
